...
static ssize_t nubia_hw_pcb_ext_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    char pcb_ext[3][12]={"NX659J_V1A","NX659J_V1S","NX659J_V1A"};
...
//if use pm gpio
	...
	nubia_pcb_gpio2_v += nubia_get_gpio_status(pcb_gpio2);
	nubia_pcb_gpio3_v += nubia_get_gpio_status(pcb_gpio3);
	multicolor_led = nubia_pcb_gpio3_v;
	syna_touch = nubia_pcb_gpio3_v;
	printk("hardware version multicolor_led = %d\n",nubia_pcb_gpio3_v);
    printk("hardware version syna_touch = %d\n",nubia_pcb_gpio3_v);
