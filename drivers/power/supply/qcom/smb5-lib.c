		...
		unsigned long event, void *data)
{
	...
	if ( evdata && evdata->data && chg ){
		...
		smblib_err(chg, "====>>>>*transition = %d, event = %x<<<<====\n", *transition, event);
		...
		if(event == MSM_HDMI_CONNECT_EVENT){
			if (*transition == MSM_HDMI_CONNECT_ON){
				smblib_err(chg, "====>>>>hdmi_connect_on = 1 MSM_HDMI_CONNECT_ON<<<<====\n");
				chg->hdmi_connect_on = 1;
			}else if (*transition == MSM_HDMI_CONNECT_OFF){	
				smblib_err(chg, "====>>>>hdmi_connect_on = 0 MSM_HDMI_CONNECT_OFF<<<<====\n");
				chg->hdmi_connect_on = 0;
			}else{
				smblib_err(chg, "MSM_HDMI_CONNECT_EVENT Do nothing.\n");
			}
		}
	}
...
#if defined(CONFIG_NUBIA_CHARGE_FEATURE)
		cancel_delayed_work_sync(&chg->step_charge_check_work);
		schedule_delayed_work(&chg->step_charge_check_work, msecs_to_jiffies(5000));
#endif
...
static void smblib_step_charge_check_work(struct work_struct *work)
{
	...
	rc = power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_GAME_JEITA, &game_jeita_pval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't get batt game_jeita_pval rc=%d\n", rc);
	}
	
	if(chg->lcd_on_limit_enable) {
		if((chg->lcd_on==1) && (temp.intval >chg->lcd_on_limit_temp)){
			if(game_jeita_pval.intval == 1){
				if(temp.intval <= 430){
					vote(chg->fcc_votable, USER_LCD_CHG, true, chg->lcd_on_limit_fcc);
				}else if((430 < temp.intval)&&(temp.intval <= 450)){
					vote(chg->fcc_votable, USER_LCD_CHG, true, 1400000);
				}else if((450 < temp.intval)&&(temp.intval <= 480)){
					vote(chg->fcc_votable, USER_LCD_CHG, true, 1000000);
				}else{
					vote(chg->fcc_votable, USER_LCD_CHG, true, 600000);
				}							
			}else{				
				vote(chg->fcc_votable, USER_LCD_CHG, true, chg->lcd_on_limit_fcc);
			}			
