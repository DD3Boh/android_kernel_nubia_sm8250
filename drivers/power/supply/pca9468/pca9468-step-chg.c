...
struct pca9468_step_chg_info {
	...
	int			jeita_step_temp_new;
	int 		jeita_step_temp_old;
	int			jeita_handle_temp_new;
	int 		jeita_handle_temp_old;
...
#define LCD_ON_CURRENT_CHARGE_STAGE0   1200000     /*2.4A*/
#define LCD_ON_CURRENT_CHARGE_STAGE1   1000000     /*2.0A*/
#define LCD_ON_CURRENT_CHARGE_STAGE2   2000000     /*4.0A*/

#define LCD_ON_TEMP_STAGE0  430
#define LCD_ON_TEMP_STAGE1  400
...
enum {
	...
	JEITA_STAGE1,	/* 10.1~15.0degree */
	JEITA_STAGE2,	/* 15.1~41.5degree - step charging */
	JEITA_STAGE3,	/* 41.6~42.5degree */
	JEITA_STAGE4,	/* 42.6~44.0degree */
	JEITA_STAGE5,	/* 44.1~53.0degree */
	JEITA_STAGE6,	/* 53.1~ 100degree  - stop charging */
	JEITA_STAGE_MAX,
};
...
static int get_step_chg_jeita_setting_from_profile(struct pca9468_step_chg_info *chip)
{
	...
	chip->jeita_step_temp_new = JEITA_STAGE0;
	chip->jeita_step_temp_old = JEITA_STAGE0;
	chip->jeita_handle_temp_new = JEITA_STAGE0;
	chip->jeita_handle_temp_old = JEITA_STAGE0;
...
static int pca9468_handle_jeita(struct pca9468_step_chg_info *chip)
{
	...
	union power_supply_propval game_jeita_pval = {0, };
	union power_supply_propval lock_screen_pval = {0, };
	...
	rc = power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_GAME_JEITA, &game_jeita_pval);
	if (rc < 0) {
		pr_err("%s: Couldn't read POWER_SUPPLY_PROP_GAME_JEITA property rc=%d\n", __func__, rc);
		return rc;
	}

	rc = power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_LOCK_SCREEN, &lock_screen_pval);
	if (rc < 0) {
		pr_err("%s: Couldn't read POWER_SUPPLY_PROP_LOCK_SCREEN property rc=%d\n", __func__, rc);
		return rc;
	}
	...
	if ((lcd_pval.intval !=1 )&&(current_index == chip->jeita_fcc_index)) {
		if(chip->lcd_off_reset == true){
			pr_info("====>>>%s: enter lcd_on =0 and current_index == chip->jeita_fcc_index<<<<====\n", __func__);
			chip->lcd_on_jeita_limit = true;
			chip->lcd_off_reset =false;
			chip->jeita_handle_temp_new = JEITA_STAGE0;
			chip->jeita_handle_temp_old = JEITA_STAGE0;
	...
	if ((chip->jeita_fcc_index == JEITA_STAGE0) ||
		...
		if (chip->jeita_fcc_index != JEITA_STAGE2) {
			if((lcd_pval.intval == 1)&&(pval.intval > chip->lcd_on_limit_temp)){
				if((game_jeita_pval.intval == 1)||(lock_screen_pval.intval == 0)){
					chip->lcd_on_jeita_limit = true;
					chip->lcd_off_reset = true;
					pr_info("====>>>%s: lock_screen_pval.intval = %d<<<<====\n", __func__, lock_screen_pval.intval);
					if(pval.intval > LCD_ON_TEMP_STAGE0){
						pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and temp > 430<<<<====\n", __func__);
						pval.intval = LCD_ON_CURRENT_CHARGE_STAGE1;
						chip->jeita_handle_temp_new = JEITA_STAGE4;
					}else if(pval.intval > LCD_ON_TEMP_STAGE1){
						pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and 40 < temp < 430<<<<====\n", __func__);
						pval.intval = LCD_ON_CURRENT_CHARGE_STAGE0;
						chip->jeita_handle_temp_new = JEITA_STAGE3;
					}else{
						pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and temp < 400<<<<====\n", __func__);
						pval.intval = LCD_ON_CURRENT_CHARGE_STAGE2;
						chip->jeita_handle_temp_new = JEITA_STAGE2;
					}					
					
					if(chip->jeita_handle_temp_old != chip->jeita_handle_temp_new){
		     			chip->jeita_handle_temp_old = chip->jeita_handle_temp_new;
						pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and set limit<<<<====\n", __func__);
					...
					if(chip->jeita_fcc_index != JEITA_STAGE5){
						chip->jeita_handle_temp_new = JEITA_STAGE0;
						chip->jeita_handle_temp_old = JEITA_STAGE0;
						chip->lcd_off_reset = true;
						if(chip->lcd_on_jeita_limit == true){
							pr_info("====>>>%s: enter lcd_on =1 and game_jeita =0 and set limit and chip->jeita_fcc_index != JEITA_STAGE5<<<<====\n", __func__);							
						...
						pr_info("====>>>%s: quit lcd_on =1 and game_jeita =0 and chip->jeita_fcc_index == JEITA_STAGE5<<<<====\n", __func__);
						chip->lcd_on_jeita_limit = true;
						chip->jeita_handle_temp_new = JEITA_STAGE0;
						chip->jeita_handle_temp_old = JEITA_STAGE0;
						/* There is no step charging */
				...
				pr_info("====>>>%s: quit lcd_on =0 or temp <400<<<<====\n", __func__);
				chip->jeita_handle_temp_new = JEITA_STAGE0;
				chip->jeita_handle_temp_old = JEITA_STAGE0;
				chip->lcd_on_jeita_limit = true;
				/* There is no step charging */
...
static int pca9468_handle_step_chg_config(struct pca9468_step_chg_info *chip)
{
	...
	if (pval.intval == 1) {
		...
		if (current_index == DC_STEP10) {
			...
			pr_info("%s: step10 - final charging done\n", __func__);
			rc = power_supply_set_property(chip->dcchg_psy,
					POWER_SUPPLY_PROP_CHARGE_DONE, &pval);
			if (rc < 0) {
				pr_err("Couldn't set %s property rc=%d\n",
					chip->dcchg_psy->desc->name, rc);
				return rc;
			}
		} else {
		...
		if((lcd_pval.intval ==1)&&(temp_pval.intval > chip->lcd_on_limit_temp)){
			if((game_jeita_pval.intval == 1)||(lock_screen_pval.intval == 0)){
				...
				pr_info("====>>>%s: lock_screen_pval.intval = %d<<<<====\n", __func__, lock_screen_pval.intval);
				if(temp_pval.intval > LCD_ON_TEMP_STAGE0){
					pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and temp > 430<<<<====\n", __func__);
					pval.intval = LCD_ON_CURRENT_CHARGE_STAGE1;
					chip->jeita_step_temp_new = JEITA_STAGE4;
				}else if(temp_pval.intval > LCD_ON_TEMP_STAGE1){
					pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and 40 < temp < 430<<<<====\n", __func__);
					pval.intval = LCD_ON_CURRENT_CHARGE_STAGE0;
					chip->jeita_step_temp_new = JEITA_STAGE3;
				}else{
					pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and temp < 400<<<<====\n", __func__);
					pval.intval = LCD_ON_CURRENT_CHARGE_STAGE2;
					chip->jeita_step_temp_new = JEITA_STAGE2;
				}
				if(chip->jeita_step_temp_old != chip->jeita_step_temp_new){
	     			chip->jeita_step_temp_old = chip->jeita_step_temp_new;					
					pr_info("====>>>%s: enter lcd_on =1 and game_jeita =1 and set limit<<<<====\n", __func__);
				...
				if(chip->lcd_on_step_chg_limit == true){				
					...
					chip->jeita_step_temp_new = JEITA_STAGE0;
					chip->jeita_step_temp_old = JEITA_STAGE0;
					pr_info("====>>>%s: enter lcd_on =1 and game_jeita =0 and set limit<<<<====\n", __func__);
					/* Need to set new input current limit and vfloat */					
					if (!is_dcchg_available(chip)) {
						pr_err("%s: Couldn't get pca9468-mains\n", __func__);
						return -ENODEV;
...
static void pca9468_status_change_work(struct work_struct *work)
{
	...
	if (chip->jeita_fcc_index == JEITA_STAGE2) {
	...
	} else if (chip->jeita_fcc_index == JEITA_STAGE5) {
		/* In jeita stage5, there is the different CV condition */
		/* Need to check charging done condition */
		/* Check charging done */
		rc = power_supply_get_property(chip->dcchg_psy,
		...
		if (prop.intval == 1) {
			pr_info("%s: charging done happens from dc\n", __func__);
			/* Set the charging done state */
			pr_info("%s: jeita_stage5 - final charging done\n", __func__);
			rc = power_supply_set_property(chip->dcchg_psy,
					POWER_SUPPLY_PROP_CHARGE_DONE, &prop);
			if (rc < 0) {
				pr_err("Couldn't set %s property rc=%d\n",
...
MODULE_DESCRIPTION("PCA9468 step charging driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0.1N");
